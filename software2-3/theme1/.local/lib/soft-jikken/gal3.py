...
class c_map:
	...
	def load(self, file_name):
		...
		try:
			mfile=open(file_name,"r", errors="ignore")
		except IOError:
...
def process_listing(map, ofile, ifile, ifn):
	...
	while(1):
		...
		if(i1>=0):
			...
			if(len(x)<2):
				if(debug>=5):
					print("Warning: malformed .globl", x, end=' ')
				ofile.write(s)
				continue
		...
		if(state==1):
			...
			if(result is not None):
				...
				# old adress is x[1]
				try:
					new_address=hex(int(x[1],base=16)-delta+f.address)
				except ValueError:
					print("ERROR: Bad address %s line %s" % (x[1],x[0]))
					break
				ofile.write("%s %s " % (x[0],new_address[2:]))
